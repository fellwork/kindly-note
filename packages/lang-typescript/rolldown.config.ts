// spec §6.2: per-package build config template (rolldown + dts).
// spec §1.2 row `@kindly-note/lang-typescript`: default-exports a deep-frozen
// LanguageDefinition produced via the typed `extendLanguage()` API; depends on
// @kindly-note/{core, lang-helpers, lang-pack-ecmascript, lang-javascript}.
import { defineConfig } from 'rolldown';
import { dts } from 'rolldown-plugin-dts';

export default defineConfig({
  input: {
    index: 'src/index.ts',
  },
  output: {
    dir: 'dist',
    format: 'esm',
    sourcemap: true,
    entryFileNames: '[name].js',
  },
  // spec §1.2: zero Node built-ins; runs on browsers, Workers, Edge, Deno, Bun, Node.
  platform: 'neutral',
  treeshake: true,
  // Every @kindly-note/* edge stays external — consumers de-dupe by import
  // resolution, no double-bundling.
  external: [/^@kindly-note\//],
  plugins: [dts()],
});
